export const testimonials = [
  {
    heading: "Fast Delivery & Great Service!",
    text: "“My helmet arrived in just two days and the quality is top-notch. BikersHub is now my go-to for all biking needs!”",
    name: "Amit Sharma",
    metas: "Purchased: Roadster Helmet",
    imageSrc: "/images/sample/sample.jpg",
    productTitle: "Roadster Helmet",
    price: "Rs.105.95",
    delay: "0s",
  },
  {
    heading: "Excellent Range of Accessories",
    text: "Found everything I needed for my new bike in one place. The staff was super helpful and the prices are unbeatable!",
    name: "Priya Verma",
    metas: "Purchased: LED Headlight Kit",
    imageSrc: "/images/sample/sample.jpg",
    productTitle: "LED Headlight Kit",
    price: "Rs.7.95",
    delay: ".1s",
  },
  {
    heading: "Quality Products, Affordable Prices",
    text: "I was impressed by the variety and quality of biking gear. Got a stylish jacket at a great price!",
    name: "Rahul Mehra",
    metas: "Purchased: Riding Jacket",
    imageSrc: "/images/sample/sample.jpg",
    productTitle: "Riding Jacket",
    price: "From Rs.18.95",
    delay: ".2s",
  },
  {
    heading: "Trusted by Bikers Everywhere",
    text: "BikersHub always delivers on time and their customer support is fantastic. Highly recommended for all riders!",
    name: "Sneha Kulkarni",
    metas: "Purchased: Touring Gloves",
    imageSrc: "/images/sample/sample.jpg",
    productTitle: "Touring Gloves",
    price: "Rs.16.95",
    delay: ".3s",
  },
];


export const thumbnails = [
  {
    imgSrc: "/images/item/tets1.jpg",
    alt: "image-product",
  },
  {
    imgSrc: "/images/item/tets2.jpg",
    alt: "image-product",
  },
];

export const testimonials2 = [
  {
    quote:
      "I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there.",
    name: "Jenifer Unix",
    purchaseItem: "Oversized Printed T-shirt",
    imgSrc: "/images/item/tets1.jpg",
    alt: "image-product",
  },
  {
    quote:
      "Fashion website is impressive! The user-friendly interface and excellent customer service make shopping a breeze.",
    name: "Robert Smith",
    purchaseItem: "The Scot Collar Mint",
    imgSrc: "/images/item/tets2.jpg",
    alt: "image-product",
  },
];

export const testimonialsData = [
  {
    imgSrc: "/images/item/tets3.jpg",
    alt: "image-product",
    name: "Robert Smith",
    item: "Patterned scarf",
    quote: `"I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing."`,
  },
  {
    imgSrc: "/images/item/tets4.jpg",
    alt: "image-product",
    name: "Jenifer Unix",
    item: "Sweetheart-neckline Top",
    quote: `"I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing."`,
  },
];

export const imagesData = [
  {
    src: "/images/item/tets3.jpg",
    alt: "image-product",
    width: 750,
    height: 851,
  },
  {
    src: "/images/item/tets4.jpg",
    alt: "image-product",
    width: 750,
    height: 851,
  },
];

export const testimonials3 = [
  {
    imgSrc: "/images/slider/te4.jpg",
    altText: "image-product",
    name: "Robert Smith",
    purchaseItem: "Boxy T-shirt",
  },
  {
    imgSrc: "/images/slider/te6.jpg",
    altText: "image-product",
    name: "Jenifer Unix",
    purchaseItem: "Sweetheart-neckline Top",
  },
];

export const slides = [
  {
    images: [
      {
        src: "/images/slider/te4.jpg",
        alt: "img-slider",
        width: 450,
        height: 513,
      },
      {
        src: "/images/slider/te3.jpg",
        alt: "img-slider",
        width: 750,
        height: 797,
      },
    ],
  },
  {
    images: [
      {
        src: "/images/slider/te6.jpg",
        alt: "img-slider",
        width: 450,
        height: 513,
      },
      {
        src: "/images/slider/te5.jpg",
        alt: "img-slider",
        width: 750,
        height: 797,
      },
    ],
  },
];

export const testimonials4 = [
  {
    iconSrc: "/images/item/quote.svg",
    heading: "OUR CUSTOMER'S REVIEWS",
    rating: 5,
    text: "Love love love! This product is so easy to use! I absolutely love the Alpha Arbutin product. It has made such a difference with my hyperpigmentation spots! I feel so much more confident without makeup on where as before I would never leave the house",
    authorName: "Robert Smith",
    authorImgSrc: "/images/slider/te4.jpg",
    purchaseLink: "#",
    purchaseItem: "100% Organic Cold-Pressed Oil",
  },
  {
    iconSrc: "/images/item/quote.svg",
    heading: "PEOPLE ARE TALKING",
    rating: 5,
    text: '"The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing."',
    authorName: "Jenifer Unix",
    authorImgSrc: "/images/slider/te6.jpg",
    purchaseLink: "#",
    purchaseItem: "Sweetheart-neckline Top",
  },
];

export const imageGroups = [
  {
    images: [
      {
        src: "/images/slider/te8.jpg",
        alt: "img-slider",
        width: 450,
        height: 513,
      },
      {
        src: "/images/slider/te7.jpg",
        alt: "img-slider",
        width: 750,
        height: 797,
      },
    ],
  },
  {
    images: [
      {
        src: "/images/slider/te10.jpg",
        alt: "img-slider",
        width: 450,
        height: 513,
      },
      {
        src: "/images/slider/te9.jpg",
        alt: "img-slider",
        width: 750,
        height: 797,
      },
    ],
  },
];

export const thumbnailSlides = [
  {
    imgSrc: "/images/item/tets5.jpg",
    altText: "image-product",
    width: 550,
    height: 585,
  },
  {
    imgSrc: "/images/item/tets6.jpg",
    altText: "image-product",
    width: 550,
    height: 585,
  },
];

export const testimonialSlides = [
  {
    quote:
      "I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there.",
    rating: 5,
    author: "Jenifer Unix",
    imageSrc: "/images/item/tets1.jpg",
    purchaseItem: "Oversized Printed T-shirt",
  },
  {
    quote:
      "Fashion website is impressive! The user-friendly interface and excellent customer service make shopping a breeze.",
    rating: 5,
    author: "Robert Smith",
    imageSrc: "/images/item/tets2.jpg",
    purchaseItem: "The Scot Collar Mint",
  },
];

export const testimonials5 = [
  {
    name: "Loretta",
    delay: "0s",
    heading: "A minimalist dream",
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
  },
  {
    name: "Sandra",
    delay: ".1s",
    heading: "Perfect meals for Mums.",
    text: "Meals are delicious and great selection to choose from. I like the large portion option & info showing nutritional value.",
  },
  {
    name: "Loretta",
    delay: ".2s",
    heading: "A minimalist dream",
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
  },
  {
    name: "Sandra",
    delay: ".3s",
    heading: "Perfect meals for Mums.",
    text: "Meals are delicious and great selection to choose from. I like the large portion option & info showing nutritional value.",
  },
  {
    name: "Loretta",
    delay: ".4s",
    heading: "A minimalist dream",
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
  },
];

export const testimonials6 = [
  {
    imgSrc: "/images/item/furniture_testimonial1.jpg",
    name: "Robert Smith",
    purchaseItem: "MR Chaise",
    review:
      "I'm impressed with the durability of the furniture collection. Even after years of use, the pieces still look and feel as good as new.",
  },
  {
    imgSrc: "/images/item/furniture_testimonial2.jpg",
    name: "Alex Graham",
    purchaseItem: "Bonbon Shade",
    review:
      "I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online.",
  },
];

export const thumbnailImages = [
  {
    src: "/images/item/furniture_testimonial1.jpg",
    alt: "image-product",
    width: 750,
    height: 703,
  },
  {
    src: "/images/item/furniture_testimonial2.jpg",
    alt: "image-product",
    width: 750,
    height: 704,
  },
];

export const testimonial3 = [
  {
    iconPath:
      "M32.4413 30.5L37.8204 19.9545L38.1913 19.2273H37.375H26.375V0.5H45.5V19.6071L39.9438 30.5H32.4413ZM6.56633 30.5L11.9454 19.9545L12.3163 19.2273H11.5H0.5V0.5H19.625V19.6071L14.0688 30.5H6.56633Z",
    heading: "OUR CUSTOMER'S RAVE REVIEWS",
    rating: 5,
    text: "I'm impressed with the durability of the furniture collection. Even after years of use, the pieces still look and feel as good as new",
    authorName: "Robert Smith",

    productName: "Racing Tricycle Baby Stroller",
    imgSrc: "/images/slider/stroller-testi1.jpg",
    imgAlt: "image-product",
    imgWidth: 550,
    imgHeight: 584,
  },
  {
    iconPath:
      "M32.4413 30.5L37.8204 19.9545L38.1913 19.2273H37.375H26.375V0.5H45.5V19.6071L39.9438 30.5H32.4413ZM6.56633 30.5L11.9454 19.9545L12.3163 19.2273H11.5H0.5V0.5H19.625V19.6071L14.0688 30.5H6.56633Z",
    heading: "OUR CUSTOMER'S RAVE REVIEWS",
    rating: 5,
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
    authorName: "Robert Smith",

    productName: "Baby Stroller with Ride-Along Board",
    imgSrc: "/images/slider/stroller-testi1.jpg",
    imgAlt: "image-product",
    imgWidth: 550,
    imgHeight: 584,
  },
];

export const thumbnailSlides2 = [
  {
    imgSrc: "/images/slider/stroller-testi1.jpg",
    imgAlt: "img-slider",
    imgWidth: 550,
    imgHeight: 584,
  },
  {
    imgSrc: "/images/slider/stroller-testi2.jpg",
    imgAlt: "img-slider",
    imgWidth: 550,
    imgHeight: 584,
  },
];

export const testimonialData = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    authorName: "Robert Smith",
    authorMeta: "Customer from USA",
    productImgSrc: "/images/products/decor-17.jpg",
    productTitle: "Jersey thong body",
    productPrice: "$105.95",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    authorName: "Allen Lyn",
    authorMeta: "Customer from France",
    productImgSrc: "/images/products/decor-12.jpg",
    productTitle: "Cotton jersey top",
    productPrice: "$7.95",
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "I finally found a web fashion site with stylish and flattering options in my size.",
    authorName: "Peter Rope",
    authorMeta: "Customer from USA",
    productImgSrc: "/images/products/decor-1.jpg",
    productTitle: "Ribbed modal T-shirt",
    productPrice: "From $18.95",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    authorName: "Hellen Ase",
    authorMeta: "Customer from Japan",
    productImgSrc: "/images/products/decor-4.jpg",
    productTitle: "Customer from Japan",
    productPrice: "$16.95",
  },
];

export const testimonials7 = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "I always find something stylish and affordable on this web fashion site",
    authorName: "Robert Smith",
    authorMeta: "Customer from USA",
    productImage: "/images/collections/electronic-16.jpg",
    productTitle: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
    productPrice: "$105.95",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    authorName: "Allen Lyn",
    authorMeta: "Customer from France",
    productImage: "/images/collections/electronic-12.png",
    productTitle: "SoundForm Rise",
    productPrice: "$7.95",
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "I finally found a web fashion site with stylish and flattering options in my size.",
    authorName: "Peter Rope",
    authorMeta: "Customer from USA",
    productImage: "/images/collections/electronic-8.png",
    productTitle: "UltraGlass 2 Treated Screen Protector for iPhone 15 Pro",
    productPrice: "From $18.95",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    authorName: "Hellen Ase",
    authorMeta: "Customer from Japan",
    productImage: "/images/collections/electronic-10.png",
    productTitle: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
    productPrice: "$16.95",
  },
];

export const testimonials8 = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    author: {
      name: "Robert Smith",
      meta: "Customer from USA",
    },
    product: {
      imgSrc: "/images/collections/setup-gear-1.jpg",
      imgAlt: "",
      title: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
      price: "$105.95",
    },
    delay: "0s",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "“ I love the variety of styles and the high-quality clothing on this web fashion site.”",
    author: {
      name: "Allen Lyn",
      meta: "Customer from France",
    },
    product: {
      imgSrc: "/images/collections/setup-gear-2.jpg",
      imgAlt: "",
      title: "SoundForm Rise",
      price: "$7.95",
    },
    delay: ".1s",
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "“ I finally found a web fashion site with stylish and flattering options in my size.”",
    author: {
      name: "Peter Rope",
      meta: "Customer from USA",
    },
    product: {
      imgSrc: "/images/collections/setup-gear-3.jpg",
      imgAlt: "",
      title: "UltraGlass 2 Treated Screen Protector for iPhone 15 Pro",
      price: "From $18.95",
    },
    delay: ".2s",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "“ I love the variety of styles and the high-quality clothing on this web fashion site.”",
    author: {
      name: "Hellen Ase",
      meta: "Customer from Japan",
    },
    product: {
      imgSrc: "/images/collections/setup-gear-4.jpg",
      imgAlt: "",
      title: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
      price: "$16.95",
    },
    delay: ".3s",
  },
];

export const slides2 = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    author: { name: "Robert Smith", metas: "Customer from USA" },
    product: {
      imgSrc: "/images/item/lookbook-item-1.jpg",
      title: "Fry Pan",
      price: "$105.95",

      imgWidth: 160,
      imgHeight: 120,
    },
    wowDelay: "0s",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    author: { name: "Allen Lyn", metas: "Customer from France" },
    product: {
      imgSrc: "/images/item/lookbook-item-2.jpg",
      title: "Whistling Tea Kettle",
      price: "$7.95",

      imgWidth: 160,
      imgHeight: 120,
    },
    wowDelay: ".1s",
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "I finally found a web fashion site with stylish and flattering options in my size.",
    author: { name: "Peter Rope", metas: "Customer from USA" },
    product: {
      imgSrc: "/images/item/lookbook-item-3.jpg",
      title: "Linen Apron",
      price: "From $18.95",

      imgWidth: 160,
      imgHeight: 120,
    },
    wowDelay: ".2s",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    author: { name: "Hellen Ase", metas: "Customer from Japan" },
    product: {
      imgSrc: "/images/item/lookbook-item-4.jpg",
      title: "Sauce Pan",
      price: "$16.95",

      imgWidth: 70,
      imgHeight: 53,
    },
    wowDelay: ".3s",
  },
];

export const testimonials9 = [
  {
    quoteSrc: "/images/item/quote.svg",
    heading: "OUR CUSTOMERS' RAVE REVIEWS",
    text: `"I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing."`,
    imgSrc: "/images/slider/te4.jpg",
    author: "Robert smith",
    product: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
  },
  ,
  {
    quoteSrc: "/images/item/quote.svg",
    heading: "BEST ONLINE FASHION SITE",
    text: `"I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing."`,
    imgSrc: "/images/slider/te4.jpg",
    author: "Robert smith",
    product: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
  },
  {
    quoteSrc: "/images/item/quote.svg",
    heading: "GREAT SELECTION AND QUALITY",
    text: `"I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing."`,
    imgSrc: "/images/slider/te4.jpg",
    author: "Robert smith",
    product: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
  },
];

export const imageGroups2 = [
  [
    {
      src: "/images/slider/te12.jpg",
      alt: "img-slider",
      width: 450,
      height: 513,
    },
    {
      src: "/images/slider/te11.jpg",
      alt: "img-slider",
      width: 750,
      height: 797,
    },
  ],
  [
    {
      src: "/images/slider/te14.jpg",
      alt: "img-slider",
      width: 450,
      height: 513,
    },
    {
      src: "/images/slider/te13.jpg",
      alt: "img-slider",
      width: 750,
      height: 797,
    },
  ],
  [
    {
      src: "/images/slider/te16.jpg",
      alt: "img-slider",
      width: 309,
      height: 352,
    },
    {
      src: "/images/slider/te15.jpg",
      alt: "img-slider",
      width: 450,
      height: 478,
    },
  ],
];

export const testimonialData2 = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    authorName: "Robert Smith",
    authorMeta: "Customer from USA",
    imgSrc: "/images/products/paddle-boards-1.jpg",
    productTitle: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
    price: "$105.95",
  },
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    authorName: "Allen Lyn",
    authorMeta: "Customer from USA",
    imgSrc: "/images/products/paddle-boards-9.jpg",
    productTitle: "ISUP PADDLE BOARD",
    price: "$105.95",
  },
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    authorName: "Allen Lyn",
    authorMeta: "Customer from USA",
    imgSrc: "/images/products/paddle-boards-11.jpg",
    productTitle: "KAYAK ISUP PADDLE ADAPTER",
    price: "$105.95",
  },
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    authorName: "Allen Lyn",
    authorMeta: "Customer from USA",
    imgSrc: "/images/products/paddle-boards-8.jpg",
    productTitle: "ALL AROUND 9' ULTRA™",
    price: "$105.95",
  },
];

export const testimonials10 = [
  {
    id: 1,
    delay: "0s",
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    author: {
      name: "Robert smith",
      metas: "Customer from USA",
    },
    product: {
      imgSrc: "/images/products/pod-store-5.jpg",
      title: "Jersey thong body",
      price: "$105.95",
    },
  },
  {
    id: 2,
    delay: ".1s",
    heading: "Great Selection and Quality",
    text: "“ I love the variety of styles and the high-quality clothing on this web fashion site.”",
    author: {
      name: "Allen Lyn",
      metas: "Customer from France",
    },
    product: {
      imgSrc: "/images/products/pod-store-8.jpg",
      title: "Cotton jersey top",
      price: "$7.95",
    },
  },
  {
    id: 3,
    delay: ".2s",
    heading: "Best Customer Service",
    text: "“ I finally found a web fashion site with stylish and flattering options in my size.”",
    author: {
      name: "Peter Rope",
      metas: "Customer from USA",
    },
    product: {
      imgSrc: "/images/products/pod-store-9.jpg",
      title: "Ribbed modal T-shirt",
      price: "From $18.95",
    },
  },
  {
    id: 4,
    delay: ".3s",
    heading: "Great Selection and Quality",
    text: "“ I love the variety of styles and the high-quality clothing on this web fashion site.”",
    author: {
      name: "Hellen Ase",
      metas: "Customer from Japan",
    },
    product: {
      imgSrc: "/images/products/pod-store-10.jpg",
      title: "Customer from Japan",
      price: "$16.95",
    },
  },
];

export const testimonials11 = [
  {
    id: 1,
    imgSrc: "/images/item/tes-wear1.jpg",
    rating: 5,
    text: "I am so happy to find a web fashion site that offers a great selection of stylish and affordable plus size clothing.",
    authorName: "Anna Lan",
    authorMeta: "Customer from Japan",
    product: {
      imgSrc: "/images/products/brown-11.jpg",
      title: "Crossover Leggings",
      price: "$38.00",
      link: "#",
    },
  },
  {
    id: 2,
    imgSrc: "/images/item/tes-wear2.jpg",
    rating: 5,
    text: "Absolutely love this women's athletic wear – the fabric is breathable, the fit is perfect, and the stylish design adds a confident flair to my workout routine.",
    authorName: "Telna Swith",
    authorMeta: "Customer from USA",
    product: {
      imgSrc: "/images/products/white-17.jpg",
      title: "Ruched Strappy Sports Bra",
      price: "$38.00",
      link: "#",
    },
  },
  {
    id: 3,
    imgSrc: "/images/item/tes-wear3.jpg",
    rating: 5,
    text: "This sportswear for women strikes the ideal balance between comfort and performance, making my workouts not only more enjoyable but also more effective.",
    authorName: "Parane Taylor",
    authorMeta: "Parane Taylor",
    product: {
      imgSrc: "/images/products/blue-5.jpg",
      title: "Pocket Shorts",
      price: "$38.00",
      link: "#",
    },
  },
  {
    id: 4,
    imgSrc: "/images/item/tes-wear4.jpg",
    rating: 5,
    text: "The durability of this women's activewear is impressive; it has quickly become my go-to choice for intense workouts without compromising on style.",
    authorName: "Robert Smith",
    authorMeta: "Customer from USA",
    product: {
      imgSrc: "/images/products/sports-clothing-6.jpg",
      title: 'Crest 7" Shorts',
      price: "$38.00",
      link: "#",
    },
  },
];

export const testimonials12 = [
  {
    heading: "AS SEE IN",
    content:
      "“Meet the formal industrial designer who now makes the bags we all want and need.”",
  },
  {
    heading: "AS SEE IN",
    content:
      "I love the variety of styles and the high-quality clothing on this web fashion site.",
  },
  {
    heading: "AS SEE IN",
    content:
      "I finally found a web fashion site with stylish and flattering options in my size.",
  },
];
export const testimonials18 = [
  {
    heading: "FEATURED IN",
    content: `Scratch is fantastic! Tooska has always had tummy issues,
                      and we’ve gone through <br class="d-md-block d-none" />
                      countless bags of food trying to find the right one.`,
  },
  {
    heading: "FEATURED IN",
    content: `Scratch is fantastic! Tooska has always had tummy issues,
                    and we’ve gone through <br class="d-md-block d-none" />
                    countless bags of food trying to find the right one.`,
  },
  {
    heading: "FEATURED IN",
    content: `Scratch is fantastic! Tooska has always had tummy issues,
                    and we’ve gone through <br class="d-md-block d-none" />
                    countless bags of food trying to find the right one.`,
  },
];

export const brandThumbnails = [
  {
    src: "/images/item/thumb-brand1.png",
    alt: "brand",
    width: 237,
    height: 32,
  },
  {
    src: "/images/item/thumb-brand2.png",
    alt: "brand",
    width: 135,
    height: 21,
  },
  {
    src: "/images/item/thumb-brand3.png",
    alt: "brand",
    width: 109,
    height: 21,
  },
];

export const testimonials13 = [
  {
    heading: "Perfect meals for Mums.",
    text: "Really good service. Everything is sent out on Thursdays, comes packed neatly alongside ice packs so the food is always kept cold even if no one is home to collect it.",
    author: "Robert smith",
  },
  {
    heading: "Delicious and filling",
    text: "I love the incredibly diverse range of meals that Soulara offers! You can filter the list of meals by ingredients (e.g. no chilli or no nuts) and there's still heaps of options.",
    author: "Jesicca A.",
  },
  {
    heading: "Majority and fantastic!",
    text: "Meals are delicious and great selection to choose from. I like the large portion option & info showing nutritional value. A little pricey but worth it if you can afford.",
    author: "Taylor A",
  },
  {
    heading: "Best Online Fashion Site",
    text: "I am so happy to find a web fashion site that offers a great selection of stylish and affordable plus size clothing.",
    author: "Jenifer S",
  },
];

export const testimonials14 = [
  {
    imgSrc: "/images/item/test-sock.jpg",
    imgAlt: "",
    imgWidth: 488,
    imgHeight: 620,
    rating: 5,
    text: "“I always find something stylish and affordable on this web fashion site”",
    author: "Robert Smith",
    meta: "Customer from USA",
    productImgSrc: "/images/item/dot-test-sock.jpg",
    productImgAlt: "",
    productImgWidth: 70,
    productImgHeight: 70,
    productTitle: "Crossover Leggings",
    productPrice: "$38.00",
    productLink: "#",
  },
  {
    imgSrc: "/images/item/test-sock2.jpg",
    imgAlt: "",
    imgWidth: 488,
    imgHeight: 620,
    rating: 5,
    text: "“I always find something stylish and affordable on this web fashion site”",
    author: "Robert Smith",
    meta: "Customer from USA",
    productImgSrc: "/images/products/sock15.jpg",
    productImgAlt: "",
    productImgWidth: 360,
    productImgHeight: 360,
    productTitle: "Crossover Leggings",
    productPrice: "$38.00",
    productLink: "#",
  },
  {
    imgSrc: "/images/item/test-sock.jpg",
    imgAlt: "",
    imgWidth: 488,
    imgHeight: 620,
    rating: 5,
    text: "“I always find something stylish and affordable on this web fashion site”",
    author: "Robert Smith",
    meta: "Customer from USA",
    productImgSrc: "/images/products/sock14.jpg",
    productImgAlt: "",
    productImgWidth: 360,
    productImgHeight: 360,
    productTitle: "Crossover Leggings",
    productPrice: "$38.00",
    productLink: "#",
  },
];

export const testimonials15 = [
  {
    id: 1,
    name: "Loretta",
    delay: "0s",
    heading: "A minimalist dream",
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
  },
  {
    id: 2,
    name: "Sandra",
    delay: ".1s",
    heading: "Perfect meals for Mums.",
    text: "Meals are delicious and great selection to choose from. I like the large portion option & info showing nutritional value.",
  },
  {
    id: 3,
    name: "Loretta",
    delay: ".2s",
    heading: "A minimalist dream",
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
  },
  {
    id: 4,
    name: "Sandra",
    delay: ".3s",
    heading: "Perfect meals for Mums.",
    text: "Meals are delicious and great selection to choose from. I like the large portion option & info showing nutritional value.",
  },
  {
    id: 5,
    name: "Loretta",
    delay: ".4s",
    heading: "A minimalist dream",
    text: "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
  },
];

export const testimonials16 = [
  {
    quoteSrc: "/images/item/quote.svg",
    rating: 5,
    text: `I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing.`,
    authorImg: "/images/item/tets3.jpg",
    authorName: "Robert Smith",
  },
  {
    quoteSrc: "/images/item/quote.svg",
    rating: 5,
    text: `I have been shopping with this web fashion site for over a year now and I can confidently say it is the best online fashion site out there. The shipping is always fast and the customer service team is friendly and helpful. I highly recommend this site to anyone looking for affordable clothing.`,
    authorImg: "/images/item/tets4.jpg",
    authorName: "Jenifer Unix",
  },
];

export const thumbImages = [
  {
    imgSrc: "/images/item/tets3.jpg",
    alt: "image-product",
    width: 750,
    height: 851,
  },
  {
    imgSrc: "/images/item/tets4.jpg",
    alt: "image-product",
    width: 750,
    height: 851,
  },
];

export const testimonials17 = [
  {
    id: 1,
    title: "OUR CUSTOMER’S REVIEWS",
    rating: 5,
    text: "Consider if the headphones support any advanced audio technologies, such as noise cancellation or surround sound, depending on your preferences. Comfort is crucial for long listening sessions.",
    author: "Robert Smith",

    productTitle: "3-in-1 Wireless Charging",
    imgSrc: "/images/slider/accessories-tes1.png",
    imgAlt: "image-product",
  },
  {
    id: 2,
    title: "OUR CUSTOMER’S REVIEWS",
    rating: 5,
    text: "Love love love! This product is so easy to use! I absolutely love the this product. High-quality headphones deliver a balanced and clear sound. Look for headphones with good bass, clear mids, and crisp highs.",
    author: "Robert Smith",

    productTitle: "Beats Studio Buds",
    imgSrc: "/images/slider/accessories-tes2.png",
    imgAlt: "image-product",
  },
];

export const thumbSlides = [
  {
    id: 1,
    imgSrc: "/images/slider/accessories-tes1.png",
    imgAlt: "img-slider-1",
  },
  {
    id: 2,
    imgSrc: "/images/slider/accessories-tes2.png",
    imgAlt: "img-slider-2",
  },
];

export const testimonials19 = [
  {
    delay: "0s",
    heading: "Incredibly Soft Skin",
    text: `“After using this skincare set, my skin has become incredibly soft and radiant. I feel more confident stepping out without heavy makeup.”`,
    author: "Robert smith",
    location: "Customer from USA",
    imgSrc: "/images/products/cosmetic7.jpg",
    productTitle: "Lip Butter Balm for Hydration & Shine",
    price: "$24.00",
  },
  {
    delay: ".1s",
    heading: "Ideal for All Skins",
    text: `"These cosmetics truly suit all skin types, from oily to dry. I no longer worry about finding the right products as they are gentle, effective, and perfect for everyday use."`,
    author: "Allen Lyn",
    location: "Customer from France",
    imgSrc: "/images/products/cosmetic5.jpg",
    productTitle: "Mini Self Tanning Natural Glow",
    price: "$18.00",
  },
  {
    delay: ".2s",
    heading: "Noticeable Change in Week",
    text: `"I noticed a remarkable change in my skin after just one week of use. It became brighter, smoother, and the fine lines significantly diminished. Truly surprising!"`,
    author: "Peter Rope",
    location: "Customer from USA",
    imgSrc: "/images/products/cosmetic3.jpg",
    productTitle: "Sephora Colorful® Eyeshadow",
    price: "$18.00",
  },
  {
    delay: ".3s",
    heading: "Perfect Skincare Solution",
    text: `"This set provides everything I need for my daily skincare routine. From cleanser to toner and moisturizer, all products are amazing and deliver noticeable results."`,
    author: "Hellen Ase",
    location: "Customer from Japan",
    imgSrc: "/images/products/cosmetic9.jpg",
    productTitle: "Unreal Liquid Blush",
    price: "$24.00",
  },
];

export const testimonialData3 = [
  {
    icon: "/images/slider/icon1.svg",
    heading: "OUR CUSTOMER'S RAVE REVIEWS",
    text: "I've ordered from a lot of places, a lot! and I must say that this place here has the best shipping experience ever. Thank you guys so much for this ♥️",
    author: "Vincent Pahm",
    purchaseItem: "Aglaonema Siam",
    imgSrc: "/images/slider/stroller-testi1.jpg",
    rating: 5,
  },
  {
    icon: "/images/slider/icon1.svg",
    heading: "UR CUSTOMER'S RAVE REVIEWS",
    text: "Love my new indoor plant! It arrived healthy and adds a refreshing touch to my home. Super easy to care for, highly recommended!",
    author: "Robert Smith",
    purchaseItem: "Baby Stroller with Ride-Along Board",
    imgSrc: "/images/slider/stroller-testi1.jpg",
    rating: 5,
  },
];
export const imageData = [
  "/images/slider/plant-testi1.jpg",
  "/images/slider/plant-testi2.jpg",
];

export const testimonials20 = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    author: "Robert smith",
    metas: "Customer from USA",
    imgSrc: "/images/products/sw-pink1.jpg",
    productTitle: "Aloha Beach Short",
    price: "$79.95",
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "I love the variety of styles and the high-quality clothing on this web fashion site.",
    author: "Kanly Mike",
    metas: "Customer from USA",
    imgSrc: "/images/products/sw-black1.jpg",
    productTitle: "Eco Essentials Cross Front Moulded Cup Bra",
    price: "$94.95",
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "I finally found a web fashion site with stylish and flattering options in my size.",
    author: "Xengliu Tia",
    metas: "Customer from China",
    imgSrc: "/images/products/sw-black9.jpg",
    productTitle: 'Lotus 7" Boardshort',
    price: "$53.97",
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "I finally found a web fashion site with stylish and flattering options in my size.",
    author: "Xengliu Tia",
    metas: "Customer from China",
    imgSrc: "/images/products/sw-black5.jpg",
    productTitle: 'Eco Essentials 7" Boardshort',
    price: "$69.95",
  },
];

export const testimonials21 = [
  {
    rating: 5,
    heading: "Best Online Fashion Site",
    text: "“ I always find something stylish and affordable on this web fashion site ”",
    author: "Robert Smith",
    metas: "Customer from USA",
    product: {
      imgSrc: "/images/products/footwear-8.jpg",
      title: "Adult Voyageur Sparrow",
      price: "$105.95",
    },
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "“ I love the variety of styles and the high-quality clothing on this web fashion site ”",
    author: "Allen Lyn",
    metas: "Customer from France",
    product: {
      imgSrc: "/images/products/footwear-9.jpg",
      title: "Jet High Top",
      price: "$7.95",
    },
  },
  {
    rating: 5,
    heading: "Best Customer Service",
    text: "“ I finally found a web fashion site with stylish and flattering options in my size ”",
    author: "Peter Rope",
    metas: "Customer from USA",
    product: {
      imgSrc: "/images/products/footwear-10.jpg",
      title: "Kids Voyageur Sneaker",
      price: "From $18.95",
    },
  },
  {
    rating: 5,
    heading: "Great Selection and Quality",
    text: "“ I love the variety of styles and the high-quality clothing on this web fashion site ”",
    author: "Hellen Ase",
    metas: "Customer from Japan",
    product: {
      imgSrc: "/images/products/footwear-11.jpg",
      title: "3-in-1 Wireless Charger with Official MagSafe Charging 15W",
      price: "$16.95",
    },
  },
];

export const testimonials22 = [
  {
    imgSrc: "/images/item/tes-personalized-pod-1.jpg",
    alt: "Testimonial 1",
    rating: 5,
    text: "Personalized Christmas gifts are amazing! They add a heartfelt touch, making every gift truly unique and memorable.",
    author: {
      name: "Robert smith",
      metas: "Customer from USA",
    },
    product: {
      imgSrc: "/images/item/tes-personalized-pod-2.jpg",
      title: "Family Ornament",
      price: { original: "$22.00", discounted: "$19.00" },
    },
  },
  {
    imgSrc: "/images/item/tes-personalized-pod-3.jpg",
    alt: "Testimonial 2",
    rating: 5,
    text: "Everyone loved their personalized Christmas gifts! From custom ornaments to engraved mugs, they felt special and appreciated.",
    author: {
      name: "Allen Lyn",
      metas: "Customer from France",
    },
    product: {
      imgSrc: "/images/item/tes-personalized-pod-4.jpg",
      title: "Customer from France",
      price: { original: "$222.00", discounted: "$199.00" },
    },
  },
  {
    imgSrc: "/images/item/tes-personalized-pod-5.jpg",
    alt: "Testimonial 3",
    rating: 5,
    text: "These gifts are more than items; they’re memories. Personalized Christmas presents made our holiday extra meaningful and joyful.",
    author: {
      name: "Uzumaki Naruto",
      metas: "Customer from France",
    },
    product: {
      imgSrc: "/images/item/tes-personalized-pod-6.jpg",
      title: "Christmas Tree Hanger",
      price: { original: "$22.00", discounted: "$19.00" },
    },
  },
  {
    imgSrc: "/images/item/tes-personalized-pod-7.jpg",
    alt: "Testimonial 4",
    rating: 5,
    text: "These gifts are more than items; they’re memories. Personalized Christmas presents made our holiday extra meaningful and joyful.",
    author: {
      name: "Uzumaki Naruto",
      metas: "Customer from France",
    },
    product: {
      imgSrc: "/images/item/tes-personalized-pod-8.jpg",
      title: "Snowflake Family",
      price: { original: "$22.00", discounted: "$19.00" },
    },
  },
];

export const testimonials23 = [
  {
    id: 1,
    rating: 5,
    heading: "Great Value for the Quality",
    text: "“Reasonably priced for the quality, an excellent choice for anyone passionate about pickleball.”",
    author: "Robert Smith",
    metas: "Customer from USA",
    productImg: "/images/products/pickleball-black-1.jpg",
    productTitle: "Franklin Signature Pickleball Paddle",
    oldPrice: "$139.99",
    newPrice: "$105.95",
  },
  {
    id: 2,
    rating: 5,
    heading: "Lightweight and Precise",
    text: "“The paddle feels super lightweight and easy to control, allowing me to make more precise shots.”",
    author: "Hellen Ase",
    metas: "Customer from Japan",
    productImg: "/images/products/pickleball-red-1.jpg",
    productTitle: "Selkirk LUXX Control Air Invikta Pickleball Paddle",
    oldPrice: null,
    newPrice: "$249.99",
  },
  {
    id: 3,
    rating: 5,
    heading: "Built to Last",
    text: "“Durable design and high-quality materials, great for both beginners and seasoned players.”",
    author: "Peter Rope",
    metas: "Customer from USA",
    productImg: "/images/products/pickleball-red-2.jpg",
    productTitle:
      "Selkirk Vanguard Power Air Invikta Midweight Pickleball Paddle",
    oldPrice: null,
    newPrice: "From $18.95",
  },
  {
    id: 4,
    rating: 5,
    heading: "Comfortable Grip for Long Play",
    text: "“The grip is soft and comfortable, no hand strain even after long play sessions.”",
    author: "Allen Lyn",
    metas: "Customer from France",
    productImg: "/images/products/pickleball-black-2.jpg",
    productTitle: "Gearbox Pro Power Elongated Pickleball Paddle",
    oldPrice: null,
    newPrice: "$274.99",
  },
];

export const testimonials24 = [
  {
    delay: "0s",
    rating: 5,
    heading: "Great Value for the Quality",
    text: "Reasonably priced for the quality, an excellent choice for anyone passionate about pickleball.",
    author: "Robert smith",
    metas: "Customer from USA",
    productImage: "/images/products/book-store-1.jpg",
    productTitle: "Burke Clete",
    oldPrice: "$139.99",
    newPrice: "$105.95",
  },
  {
    delay: ".1s",
    rating: 5,
    heading: "Lightweight and Precise",
    text: "The paddle feels super lightweight and easy to control, allowing me to make more precise shots.",
    author: "Hellen Ase",
    metas: "Customer from Japan",
    productImage: "/images/products/pickleball-red-2.jpg",
    productTitle: "Funny a novel Story",
    price: "$249.99",
  },
  {
    delay: ".2s",
    rating: 5,
    heading: "Built to Last",
    text: "Durable design and high-quality materials, great for both beginners and seasoned players.",
    author: "Peter Rope",
    metas: "Customer from USA",
    productImage: "/images/products/book-store-3.jpg",
    productTitle: "Everytime Vacation Some",
    price: "From $18.95",
  },
  {
    delay: ".3s",
    rating: 5,
    heading: "Comfortable Grip for Long Play",
    text: "The grip is soft and comfortable, no hand strain even after long play sessions.",
    author: "Allen Lyn",
    metas: "Customer from France",
    productImage: "/images/products/book-store-4.jpg",
    productTitle: "Gearbox Pro Power Elongated Pickleball Paddle",
    price: "$274.99",
  },
];

export const testimonials25 = [
  {
    id: 1,
    title: "OUR CUSTOMER'S RAVE REVIEWS",
    rating: 5,
    review:
      "This four-button console is a perfect throwback! Simple design with classic games like Contra and Mario—great for reliving childhood memories.",
    author: "Vincent Pahm",
    product: "logitech G29 Driving Force Wired Controller",
    imgSrc: "/images/slider/gaming-accessories-testi1.jpg",
  },
  {
    id: 2,
    title: "UR CUSTOMER'S RAVE REVIEWS",
    rating: 5,
    review:
      "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
    author: "Robert Smith",
    product: "Baby Stroller with Ride-Along Board",
    imgSrc: "/images/slider/gaming-accessories-testi2.jpg",
  },
  {
    id: 3,
    title: "UR CUSTOMER'S RAVE REVIEWS",
    rating: 5,
    review:
      "Exceptional quality at a fraction of the price you would pay for big brands. Very elegant, discreet and beautiful materials.",
    author: "Robert Smith",
    product: "Baby Stroller with Ride-Along Board",
    imgSrc: "/images/slider/gaming-accessories-testi3.jpg",
  },
];
